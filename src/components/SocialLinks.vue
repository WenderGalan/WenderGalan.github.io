<template>
  <div class="flex flex-wrap items-center" :class="sizeClass">
    <a
      ...
      class="flex items-center justify-center rounded-full border border-slate-200/40 text-slate-600 transition hover:border-[var(--color-primary)] hover:text-[var(--color-primary)] dark:border-slate-700/60 dark:text-slate-300"
      :class="buttonClass"
      :aria-label="link.label"
    >
      <span class="sr-only">{{ link.label }}</span>
      <svg :viewBox="link.viewBox" fill="currentColor" aria-hidden="true" :class="iconClass">
        <path :d="link.path" />
      </svg>
    </a>
  </div>
